...
using WoAutoCollectionPlugin.ECommons;
    ...
    internal unsafe class AutoCraftBot
    {
        ...
        private static void Framework_Update(Dalamud.Game.Framework framework)
        {
            if (AutoCraftBotEnable)
            {
                ...
                {
                    if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose("Entered materia extraction");
                    ...
                    {
                        if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose("Crafting");
                        if (Throttler.Throttle(1000))
                        {
                            if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose("Closing crafting log");
                ...
                {
                    if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose("Entered repair check");
                    if (GenericHelper.TryGetAddonByName<AtkUnitBase>("RecipeNote", out var addon) && addon->IsVisible && DalamudApi.Condition[ConditionFlag.Crafting])
                    {
                        if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose("Crafting");
                        if (Throttler.Throttle(1000))
                    ...
                    {
                        if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose("Not crafting");
                ...
                {
                    if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose("Addon visible");
                    if (Tasks.Count == 0)
                    {
                ...
                else
                {
                    ...
                    {
                        if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose("Addon invisible");
                        if (Tasks.Count == 0 && !DalamudApi.Condition[ConditionFlag.Crafting40])
                        {
                            if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose("Opening crafting log");
                            if (RecipeID == 0)
                            {
                            ...
                            {
                                if (WoAutoCollectionPlugin.Debug) PluginLog.Debug($"Opening recipe {RecipeID}");
