...
namespace WoAutoCollectionPlugin.Helper
{
    public unsafe static class RepairHelper
    {
        ...
        internal static bool ProcessRepair(bool use = true)
        {
            ...
            {
                if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose("Condition good");
                if (GenericHelper.TryGetAddonByName<AddonRepair>("Repair", out var r) && r->AtkUnitBase.IsVisible)
                {
                    if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose("Repair visible");
                    if (Throttler.Throttle(500))
                    {
                        if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose("Closing repair window");
                        ActionManager.Instance()->UseAction(ActionType.General, 6);
                    }
                    return false;
                }
                if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose("return true");
                return true;
            }
            else
            {
                if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose($"Condition bad, condition is {GetMinEquippedPercent()}, config is {20}");
                if (use)
                {
                    if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose($"Doing repair");
                    if (GenericHelper.TryGetAddonByName<AddonRepair>("Repair", out var r) && r->AtkUnitBase.IsVisible)
                    {
                    ...
                    {
                        if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose($"Repair not visible");
                        if (Throttler.Throttle(500))
                        {
                            if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose($"Opening repair");
                            ActionManager.Instance()->UseAction(ActionType.General, 6);
                        }
                    }
                }
                if (WoAutoCollectionPlugin.Debug) PluginLog.Verbose($"Returning false");
                return false;
            }
        }
    }
