...
namespace WoAutoCollectionPlugin.Gui;
...
internal class Icons : IDisposable
{
    ...
    private static TexFile? GetHdIcon(uint id)
    {
        ...
        return DalamudApi.DataManager.GetFile<TexFile>(path);
    }
    ...
    public TextureWrap LoadIcon(uint id)
    {
        ...
        var icon     = GetHdIcon(id) ?? DalamudApi.DataManager.GetIcon(id)!;
        var iconData = icon.GetRgbaImageData();

        ret        = DalamudApi.PluginInterface.UiBuilder.LoadImageRaw(iconData, icon.Header.Width, icon.Header.Height, 4);
        _icons[id] = ret;
        return ret;
    }
