...
public readonly struct CumulativeWeatherRates
{
    ...
    public readonly (Weather.Weather Weather, byte CumulativeRate)[] Rates;
    ...
    public CumulativeWeatherRates(GameData data, WeatherRate rate)
    {
        Rates = rate.UnkData0.Where(w => w.Rate > 0)
            .Select(w => data.Weathers.TryGetValue((uint)w.Weather, out var weather)
                ? (weather, w.Rate)
                : (Weather.Weather.Invalid, w.Rate))
            .ToArray();
        byte lastRate = 0;
        for (var i = 0; i < Rates.Length; ++i)
        {
            if (Rates[i].Weather.Id == Weather.Weather.Invalid.Id)
    ...
    private CumulativeWeatherRates(bool _)
        => Rates = Array.Empty<(Weather.Weather, byte)>();

    public byte ChanceForWeather(params Weather.Weather[] weathers)
    {
