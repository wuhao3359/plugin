...
public class WeatherTimeline : IComparable<WeatherTimeline>
{
    ...
    {
        var now    = WoAutoCollectionPlugin.Time.ServerTime;
    ...
    {
        var offset = List.Count > 0 ? MillisecondsPerWeather - List.Last().Offset(WoAutoCollectionPlugin.Time.ServerTime) : -MillisecondsPerWeather;
    ...
    public WeatherListing Find(IList<Weather> weather, IList<Weather> previousWeather, RepeatingInterval eorzeanHours, TimeStamp now,
        uint increment = 32)
    {
        TrimFront();
        ...
        while (true)
        {
            ...
            if (List.Last().Timestamp >= maxDate)
                return new WeatherListing(Weather.Invalid, TimeStamp.Epoch);
        }
    }
