...
using static WoAutoCollectionPlugin.Craft.CurrentCraft;

namespace WoAutoCollectionPlugin.Craft
{
    public unsafe static class CurrentCraft
    {
        ...
        {
            public const uint
                BasicSynth = 100001,        // 制作
                BasicTouch = 100002,        // 加工
                MastersMend = 100003,       // 精修
                HastyTouch = 100355,        // 仓促
                RapidSynthesis = 100363,    // 高速制作
                Observe = 100010,           // 观察
                Tricks = 100371,            // 秘诀   高品质
                WasteNot = 4631,            // 俭约
                Veneration = 19297,         // 崇敬
                StandardTouch = 100004,     // 中级加工
                GreatStrides = 260,         // 阔步
                Innovation = 19004,         // 改革
                FinalAppraisal = 19012,     // 最终确认
                WasteNot2 = 4639,           // 长期俭约
                ByregotsBlessing = 100339,  // 比格尔的祝福
                PreciseTouch = 100128,      // 集中加工 高品质
                MuscleMemory = 100379,      // 坚信   首次
                CarefulSynthesis = 100203,  // 模范制作
                Manipulation = 4574,        // 掌握
                PrudentTouch = 100227,      // 俭约加工
                FocusedSynthesis = 100235,  // 注视制作 观察
                FocusedTouch = 100243,      // 注视加工 观察
                Reflect = 100387,           // 娴静   首次
                PreparatoryTouch = 100299,  // 胚料加工
                Groundwork = 100403,        // 胚料制作
                DelicateSynthesis = 100323, // 精密制作
                IntensiveSynthesis = 100315,    // 集中制作 高品质
                TrainedEye = 100283,        // 工匠的神速    首次 低10级配方
                AdvancedTouch = 100411,     // 上级加工
                PrudentSynthesis = 100427,  // 俭约制作
                TrainedFinesse = 100435,    // 工匠的神技 娴静10以上
                CarefulObservation = 100395,    // 设计变动 图纸
                HeartAndSoul = 100419;      // 专心致志 图纸
        }
        ...
        public static class Buffs
        {
            public const ushort
                ...
                Veneration = 2226,  // 崇敬
                GreatStrides = 254, // 阔步
                Manipulation = 1164,// 掌握
                WasteNot = 252,     // 俭约
                WasteNot2 = 257,    // 长期俭约
                FinalAppraisal = 2190,  // 最终评估
                MuscleMemory = 2191;    // 坚信
        }
            ...
            set
            {
                if (value != state)
                {
                    if (state == CraftingState.Crafting)
                    {
                        ...
                        //if (!wasSuccess && Handler.Enable)
                        //{
                        //    Handler.Enable = false;
                        //    PluginLog.Error("You failed a craft. Disabling Endurance.");
                        //}

                        //if (!LastItemWasHQ && LastCraftedItem != null && !LastCraftedItem.IsCollectable && LastCraftedItem.CanBeHq && Handler.Enable)
                        //{
                        //    Handler.Enable = false;
                        //    PluginLog.Error("You crafted a non-HQ item. Disabling Endurance.");
                        //}
                    }
                }
                state = value;
        ...
        public unsafe static bool GetCraft()
        {
            ...
            try
            {
                ...
                IntPtr synthWindow = DalamudApi.GameGui.GetAddonByName("Synthesis", 1);
        ...
        public static double BaseQuality(Recipe? recipe = null)
        {
            ...
            {
                PluginLog.Error(ex, "BaseQuality");
                return 0;
            }
        }
        ...
        public static double BaseProgression()
        {
            ...
            {
                PluginLog.Error(ex, "BaseProgression");
                return 0;
            }
        }
        ...
        {
            GenericHelper.TryGetAddonByName<AddonSynthesis>("Synthesis", out var synthesis);
            ...
            if (qualityActions->ByregotsBlessing.QualityIncrease >= remainingQuality && CanUse(Skills.ByregotsBlessing))
                return Skills.ByregotsBlessing;
        ...
        {
            if (DalamudApi.ClientState.LocalPlayer is null) return null;
        ...
        public static unsafe bool BestSynthesis(out uint action, bool allowComplete = true)
        {
            ...
            if (allowComplete)
            {
                if (CanUse(Skills.IntensiveSynthesis))
                {
                    ...
                    return progressActions->IntensiveSynthesis.ProgressIncrease >= remainingProgress;
                }

                // Only use Groundwork to speed up if it'll complete
                if (progressActions->Groundwork.ProgressIncrease >= remainingProgress == true)
                {
            ...
            else
            {
                ...
                if (progressActions->IntensiveSynthesis.ProgressIncrease >= remainingProgress == false)
                {
                    action = Skills.IntensiveSynthesis;
                    return false;
                }
                ...
                if (progressActions->Groundwork.ProgressIncrease >= remainingProgress == false &&
                    (
                ...
                if (PreviousActionSameAs(Skills.Observe) && progressActions->FocusedSynthesis.ProgressIncrease >= remainingProgress == false)
                {
                    action = Skills.FocusedSynthesis;
                    return false;
                }
