...
namespace WoAutoCollectionPlugin.Classes;
...
public class Aetheryte : IComparable<Aetheryte>
{
    ...
    public Aetheryte(GameData gameData, AetheryteRow data)
    {
        ...
        var mapMarker = DalamudApi.DataManager.GetExcelSheet<MapMarker>()?.FirstOrDefault(m => m.DataType == 3 && m.DataKey == data.RowId);
        if (mapMarker == null)
        {
    ...
    public int WorldDistance(uint territoryId, int x, int y)
        => territoryId == Territory.Id
            ? Utility.Maths.SquaredDistance(x, y, XCoord, YCoord)
            : int.MaxValue;

    public int AetherDistance(ushort x, ushort y, ushort plane)
        => Utility.Maths.SquaredDistance(x, y, plane, XStream, YStream, Plane);
