...
namespace WoAutoCollectionPlugin.RawInformation
{
    public static class CharacterInfo
    {
        public static byte? CharacterLevel()
            => DalamudApi.ClientState.LocalPlayer?.Level;

        public static uint? JobID()
            => DalamudApi.ClientState.LocalPlayer?.ClassJob.Id;

        public static bool IsCrafting { get; set; }

        public static uint CurrentCP =>
            DalamudApi.ClientState.LocalPlayer.CurrentCp;

        public static uint MaxCP =>
            DalamudApi.ClientState.LocalPlayer.MaxCp;
        ...
        public static bool CanUseTrainedEye
            => DalamudApi.ClientState?.LocalPlayer?.Level >= Recipe?.RecipeLevelTable.Value?.ClassJobLevel + 10 && CurrentStep == 1 && DalamudApi.ClientState?.LocalPlayer?.Level >= 80;
    ...
    public unsafe static class CharacterStats
    {
        ...
        private unsafe static void FetchMemory()
        {
            try
            {
                ...
                {
                    getBaseParamAddress = DalamudApi.SigScanner.ScanText("E8 ?? ?? ?? ?? 44 8B C0 33 D2 48 8B CB E8 ?? ?? ?? ?? BA ?? ?? ?? ?? 48 8D 0D");
